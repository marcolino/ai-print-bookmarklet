javascript: (function () {
  ...
  /* Function to get selected text with its parent message */
  function getSelectedMessage() {
    const selection = window.getSelection();
    if (!selection || selection.toString().trim() === '') return null;
    
    /* Find the closest message container */
    let messageContainer = selection.anchorNode;
    while (messageContainer && !messageContainer.matches?.(config.messageSelector)) {
  ...
  function createPrintDocument(contentNodes, isSelection = false) {
  ...
  function prepareContent() {
    /* First check for selected text with its message container */
    const selectedMessage = getSelectedMessage();
    if (selectedMessage) {
      const isHuman = selectedMessage.matches(config.humanClass);
      ...
      const contentClone = selectedMessage.cloneNode(true);
